...
class FlexUnlimited:
  ...
  """
    ...
    def __acceptOfferID(self, ID):
        self.__requestHeaders["X-Amz-Date"] = self.__getAmzDate()
        #self.__attemptedOffers.append(offer)
    ...
    def run(self):
        ...
        while self.__retryCount < self.retryLimit:
            ...
            trytimestart = time.time_ns()
            # with open("offers.json", 'w') as outfile:
            #     json.dump(offersResponse.json(), outfile, indent=4)

            if offersResponse.status_code == 200:
                # offers_df = pd.json_normalize(offersResponse.json(), record_path=['offerList'])
                # # offers_filtered = offers_df.loc[offers_df['rateInfo.priceAmount'] >= 90]
                # offers_filtered = offers_df[offers_df['rateInfo.priceAmount'] >= 90]
                #
                # if not offers_filtered.empty:
                #     offers_filtered['offerId'].apply(self.__acceptOfferID)
                #     print(offers_filtered.to_string())
            ...
            trytimeresults.append(time.time_ns() - trytimestart)
            time.sleep(self.refreshInterval)

        Log.info("Job search cycle ending...")
        Log.info(f"Accepted {len(self.__acceptedOffers)} offers out of {len(self.__attemptedOffers)} in")
        Log.info(
            f" {(time.time() - self.__startTimestamp) / 3600} hours, current round took {(time.time() - searchTimeStamp) / 60} minutes")
        Log.info(f"Average attempt process time: {(sum(trytimeresults) / len(trytimeresults)) / 1000000}")
        if len(self.__attemptedOffers) > 0:
            Log.info(f"success rate: {len(self.__acceptedOffers) / len(self.__attemptedOffers) * 100}")
